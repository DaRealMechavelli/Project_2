...
var actorChars = {
  "@": Player,
  "=": Lava, "|": Lava, "v": Lava,
  "b": Bomb, // Bomb that kills player on touch
  "i": Invuln, // Potion that protects player from lava and bombs
  "p": Portal // Reaching a portal is the objective of the game
};
...
var bombWobbleSpeed = 10, bombWobbleDist = 0.05;
var portalWobbleSpeed = 0, portalWobbleDist = 0.00;

Bomb.prototype.act = function(step) {
  this.wobble += step * bombWobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * bombWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Invuln.prototype.act = function(step) {
...
Portal.prototype.act = function(step) {
  ...
  var wobblePos = Math.sin(this.wobble) * portalWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.finishDelay = 1;
		...
		// In setting the level status to something other than null,
		// the triggers that would lose the game are unable to fire.
		// With this, it is impossible to lose the game. Now this is true invulnerability!
		this.status = "invulnerable";
		console.log("Stand strong in the face of death!");
